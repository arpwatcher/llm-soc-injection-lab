...
injection techniques mixed into the untrusted log field, optional defensive prompting, and
a scoring pass that says how often each technique actually changes the analyst's decision.
...
- `injections.py` - seven injection techniques, each mixed into an alert's raw log field,
  ...
  filters), a fabricated ticketing-system output block, and an indirect reference to a
  made-up KB article.
- `analyst.py` - builds the prompt sent to the model: a system prompt establishing the role
  ...
  wrapping the untrusted content in clear delimiters. Supports an optional `sandwich`
  defense that repeats the "this is data" reminder again after the untrusted block.
  Parses the model's response back into a structured decision - pulls the JSON out even if
  the model wraps it in a sentence, and rejects anything outside the known action set
  instead of guessing.
- `llm_client.py` - one small interface (`complete(system_prompt, user_message) -> str`)
  ...
  server. Three fake clients model different failure modes without needing a real model
  running: `RobustFakeClient` always reads the alert honestly by keyword; `VulnerableFakeClient`
  additionally caves the moment it sees a known injection marker phrase (it does NOT catch
  the homoglyph technique, on purpose - that's the point of that technique);
  `SandwichSensitiveFakeClient` behaves like the vulnerable one but backs off if the
  sandwich defense's reminder is present, modeling recency bias. These aren't stand-ins for
...
- `cli.py` - `soclab run --client ... --defense none|sandwich` runs the battery once;
  `soclab compare --client ... [--report FILE.md]` runs it under both defenses back to
  back and optionally writes the comparison as markdown; `soclab list-techniques` lists
  what's available.
...
58 tests, all deterministic - no network calls, nothing depends on a real model being
